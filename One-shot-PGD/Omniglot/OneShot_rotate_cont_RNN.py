...
class ContRNNChoiceLayer(lasagne.layers.MergeLayer):
    def __init__(self, incomings, W_question, W_choice,W_out,h_dim,n_classes, nonlinearity=lasagne.nonlinearities.tanh,**kwargs):
        super(ContRNNChoiceLayer, self).__init__(incomings, **kwargs) #？？？不知道这个super到底做什么的，会引入input_layers和input_shapes这些属性
        if len(incomings) != 2:
            raise NotImplementedError
        batch_size, max_sentlen ,embedding_size = self.input_shapes[0]
        embedding_size=h_dim
    ...
    def get_output_shape_for(self, input_shapes):
        return (self.batch_size,self.max_sentlen)
    def get_output_for(self, inputs, **kwargs):
        '''input[0]是memory[bs*path_length,n_classes,h_dim+n_class]
           input[1]是hidden[bs*path_length,1,h_dim+n_class]'''

        xxx=inputs[1].repeat(self.n_classes,axis=1)[:,:,self.h_dim:]-inputs[0][:,:,self.h_dim:]
        # 先给它们做减法，得到了　bs*path_length,n_classes（这其实是memory的大小）,n_classes（rnn中类别多少形成的rnn的长度） 的东西
        xxx=xxx.reshape([-1,self.n_classes,1]) #适应rnn网络的输入（None,length,1）这样的东西
        rnn_out=lasagne.layers.helper.get_output(self.network,{self.l_in:xxx})
        rnn_out=rnn_out.reshape([self.batch_size,self.max_sentlen])
        beta=lasagne.nonlinearities.softmax(rnn_out) #(BS,max_sentlen)
        return beta
class Model:
    ...
    def build(self):
        ...
        l_range_dense2 = lasagne.layers.DenseLayer(l_range_flatten,tmp_h_dim,W=D1,nonlinearity=lasagne.nonlinearities.rectify) #[bs*path_length,dimension]
        ...
        if 1:
            ...
            if if_cont==1:
                ...
                l_range_status=ContRNNChoiceLayer((l_range_memory,l_range_hidden),D3,D3,D3,self.h_dim-self.n_classes,self.n_classes,nonlinearity=lasagne.nonlinearities.tanh) #[bs*pl,(n_class+1),dim]
            else:
