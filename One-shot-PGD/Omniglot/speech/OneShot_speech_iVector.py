...
import speech_1of5_matload_ivector as speech
...
class Model:
    ...
    def sample_one_path(self,now_state,now_memory_label, prob,this_label,idx_path_length,path_length,yy_truth):
    ...
    def train(self):
        ...
        if 0:
            ...
            ppparmas='speech_params/validbest_cnn_fisher_0_validacc0.742943548387_2017-08-23 15:02:12.pklz' #　0.755040322581 	749.0 992
            ...
            ppparmas='speech_params/speech_0.0312583775362_batchnorm_12345aver_0.5_triplet_2017-07-07 00:11:41.pklz' #　0.829637096774 	823.0 992
            print ppparmas
        ...
        for epoch in range(self.n_epoch):
                ...
                # 初始化两个循环的参数，state和概率
                    ...
                    for t in range(path_length):  # 进行了path_length步
                        for idx, prob in enumerate(total_probs[:, t, :]):  # 对于batch里的每一个样本
                            ...
                            action, new_memory_state, reward, new_memory_label = self.sample_one_path(now_state,now_memory_label, prob,yy_batch[idx,t],t,path_length,yy_batch[idx,t])
                    ...
                    print '\n\n\niter:',epoch,repeat_time,idx_batch,'\tcost:{},average_reward:{},time:{}'.format(cost,aver_reward,time.time()-batch_start_time),time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    ...
                    if output_label=='1' and fre==0 and idx_batch%200==0:
            ...
            try:
                ...
                pickle.dump(prev_weights,open('speech_params/params_{}_{}_{}_{}_{}'.format(save_path,epoch,repeat_time,tmp_reward/batch_total_number,time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())),'wb'))
            except:
                pass
...
if __name__=='__main__':
    ...
    if test_mode:
        ...
        parser.add_argument('--x_dimension', type=tuple, default=(100,1), help='Dimension#')
    parser.add_argument('--h_dimension', type=int, default=100, help='Dimension#')
    parser.add_argument('--tmp_h_dim', type=int, default=100, help='tmp_h_Dimension#')
    ...
    parser.add_argument('--save_path', type=str, default='ivector_1of5', help='Task#')
