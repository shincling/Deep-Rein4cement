...
class Model:
    ...
    def build(self):
        ...
        self.x_range_shared=theano.shared(np.zeros((self.n_paths,self.batch_size,self.path_length,self.x_dim[0],self.x_dim[1]),dtype=theano.config.floatX),borrow=True)
        self.x_range_label=theano.shared(np.zeros((self.n_paths,self.batch_size,self.path_length,self.n_classes),dtype=theano.config.floatX),borrow=True)
        self.x_range_action=theano.shared(np.zeros((self.n_paths,self.batch_size,self.path_length,self.n_classes),dtype=theano.config.floatX),borrow=True)
        self.x_range_reward=theano.shared(np.zeros((self.n_paths,self.batch_size),dtype=theano.config.floatX),borrow=True)
        self.x_range_memory=theano.shared(np.zeros((self.n_paths,self.batch_size,self.path_length,self.n_classes,self.h_dim),dtype=theano.config.floatX),borrow=True)
        ...
        l_range_in = lasagne.layers.InputLayer(shape=(self.n_paths,self.batch_size,self.path_length,self.x_dim[0],self.x_dim[1]))
        l_range_flatten = lasagne.layers.ReshapeLayer(l_range_in,[self.n_paths*self.batch_size*self.path_length,1,self.x_dim[0],self.x_dim[1]])
        ...
        l_range_dense2_origin=lasagne.layers.ReshapeLayer(l_range_dense2,[self.n_paths,self.batch_size,self.path_length,self.h_dim])
        l_range_label = lasagne.layers.InputLayer(shape=(self.n_paths,self.batch_size,self.path_length,self.n_classes))
        if hid==1:
        ...
        if if_cont==1:
            l_range_dense2_origin=lasagne.layers.ConcatLayer((l_range_dense2_origin,l_range_label),axis=-1)
            self.h_dim+=self.n_classes
        l_range_hidden=lasagne.layers.ReshapeLayer(l_range_dense2_origin,[self.n_paths*self.batch_size*self.path_length,1,self.h_dim])
        ...
        l_range_memory_in = lasagne.layers.InputLayer(shape=(self.n_paths,self.batch_size,self.path_length,self.n_classes,self.h_dim))
        l_range_memory = lasagne.layers.ReshapeLayer(l_range_memory_in,[self.n_paths*self.batch_size*self.path_length,self.n_classes,self.h_dim])
        if 0:
        ...
        if 1:
            ...
            l_range_mu = lasagne.layers.ReshapeLayer(l_range_status,[self.n_paths,self.batch_size,self.path_length,self.n_classes])

        '''模型的总体参数和更新策略等'''
        hidden = lasagne.layers.helper.get_output(l_range_dense2_origin, {l_range_in: self.x_range_shared,l_range_label:self.x_range_label})
        probas_range = lasagne.layers.helper.get_output(l_range_mu, {l_range_in: self.x_range_shared,l_range_memory_in:self.x_range_memory,l_range_label:self.x_range_label})
        ...
        # }
        cost=-(1./self.n_paths)*T.sum(T.mean(T.sum(T.sum(T.log(probas_range)*self.x_range_action,axis=3),axis=2)*self.x_range_reward))
        ...
        self.output_model_range = theano.function([],[probas_range[0],cost,hidden],on_unused_input='ignore',allow_input_downcast=True)
        self.output_model_range_updates = theano.function([],[probas_range[0],cost,hidden],updates=updates,on_unused_input='ignore',allow_input_downcast=True)
        self.output_hidden = theano.function([self.x_range_shared,self.x_range_label],[hidden[0,:,0]],on_unused_input='ignore',allow_input_downcast=True)
    ...
    def train(self):
        ...
        for epoch in range(self.n_epoch):
            ...
            total_state = np.zeros([n_paths,batch_size, path_length, n_classes+1,h_dim])#整个state是memory和当前x的hidden的结合
            total_memory_label=np.zeros([n_paths,batch_size,path_length,n_classes,path_length],dtype=np.int32)-1 #取作-1，标志着还没有存放过样本
            total_reward = np.zeros([n_paths,batch_size, path_length])
            total_action = np.zeros([n_paths,batch_size, path_length])
            total_probs = np.zeros([n_paths,batch_size, path_length, n_classes])
            for repeat_time in range(n_paths):  # 每一个batch都要经过多次重复采样获取不同的道路
                ...
                # 初始化两个循环的参数，state和概率
                    ...
                    yy_batch = yy[idx_batch * batch_size:(idx_batch + 1) * batch_size]
                    # yy_batch_vector=action_to_vector_real(yy_batch,self.n_classes)
                    yy_batch_vector=action_to_vector(yy_batch,self.n_classes,1).reshape(1,batch_size,path_length,n_classes).repeat(n_paths,axis=0)
                    global same_batch
                    if same_batch:
                    ...
                    xx_batch_0=xx_batch[:,0,:].reshape([1,xx_batch.shape[0],1,xx_batch.shape[-2],xx_batch.shape[-1]])
                    xx_batch_0_repeat=xx_batch_0.repeat(path_length,axis=2).repeat(n_paths,axis=0)
                    memory_0=np.zeros((batch_size,self.n_classes,h_dim))
                    memory_0_repeat=np.zeros((n_paths,batch_size,path_length,self.n_classes,h_dim))
                    ...
                    total_state[repeat_time,:, 0, -1] =self.output_hidden(xx_batch_0_repeat,yy_batch_vector)[0]#这里是把当前（第一个）x的hidden计算出来，然后填给state
                    total_state[repeat_time,:, 0, :-1] = memory_0
                    '''state是[bs,class+1,x_dim]尺度的东西，第二个维度前class个是memory状态,最后一个是当前输入的隐层表达'''
                    total_probs[repeat_time,:, 0, :] = probs_sample_0

                    for t in range(path_length):  # 进行了path_length步
                        for idx, prob in enumerate(total_probs[repeat_time,:, t, :]):  # 对于batch里的每一个样本
                            now_state = total_state[repeat_time,idx, t].copy()
                            now_memory_label=total_memory_label[repeat_time,idx,t].copy()
