...
class ChoiceLayer(lasagne.layers.MergeLayer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        activation3=(activation3).reshape([self.batch_size,self.max_sentlen])#.dimshuffle(0,'x',2)#.repeat(self.max_sentlen,axis=1)
        # final=T.dot(activation,self.W_o) #(BS,max_sentlen)
        # if inputs[2] is not None:
        #     final=inputs[2]*final-(1-inputs[2])*1000000
        alpha=lasagne.nonlinearities.softmax(activation2) #(BS,max_sentlen)
        # beta=lasagne.nonlinearities.softmax(activation3) #(BS,max_sentlen)
        # alpha=lasagne.nonlinearities.softmax(alpha+self.ratio.repeat(self.batch_size,0).repeat(self.max_sentlen,1)*beta) #(BS,max_sentlen)
        return alpha
...
class Model:
    ...
    def build(self):
        ...
        l_range_dense2 = lasagne.layers.DenseLayer(l_pool1,300,W=D1,nonlinearity=lasagne.nonlinearities.rectify) #[bs*path_length,dimension]
        ...
        for epoch in range(self.n_epoch):
            ...
            for repeat_time in range(n_paths):  # 每一个batch都要经过多次重复采样获取不同的道路
                ...
                # 初始化两个循环的参数，state和概率
                    ...
                    yy_batch = yy[idx_batch * batch_size:(idx_batch + 1) * batch_size]
                    # yy_batch_vector=action_to_vector_real(yy_batch,self.n_classes)
                    yy_batch_vector=action_to_vector(yy_batch,self.n_classes,0.5)
