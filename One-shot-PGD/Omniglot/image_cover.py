...
def get_sequence_images_cover(train_data,test_data,train_labels,test_labels,path_length,total_label,size,total_roads=10000):
    final_x=np.zeros((total_roads,path_length,size[0],size[1]))
    final_y=np.zeros((total_roads,path_length))
    train_labels=train_data.keys()
    test_labels=test_data.keys()
    for one_sample in range(total_roads):
        label_list=random.sample(test_labels,total_label)
        train_label_list=random.sample(train_labels,total_label)
        one_shoot_label=label_list[-1]
        insert_idx=np.random.randint(0,path_length-1)
        final_x[one_sample,insert_idx]=random.sample(test_data[one_shoot_label],1)[0]
        final_y[one_sample,insert_idx]=int(one_shoot_label)
        final_x[one_sample,-1]=random.sample(test_data[one_shoot_label],1)[0]
        final_y[one_sample,-1]=int(one_shoot_label)
        for i in range(path_length-1):
            if i!=insert_idx:
                label=np.random.choice(train_label_list[:-1])
                final_y[one_sample,i]=int(label)
                final_x[one_sample,i,:]=random.sample(train_data[label],1)[0]
    return final_x,final_y
...
if cnn_only:
    ...
    x_train,y_train,x_test,y_test,train_labels,test_labels=build(path,pathdir,files,labels,all_count,ratio,size)
    del files
